...
import { RemoteFunctions } from "../../api/types";
...
const EnterQueuePage = () => {
	...
	let [passCodeVerification, setPassCodeVerification] = useState<
		...
		| "VERIFYING"
	>("INITIAL");
	let [queue, setQueue] = useState<RemoteFunctions["GET_QUEUE_WITH_PASSCODE"]["returns"]>(null);
	...
	return <form onSubmit={async event => {
		...
		{queue === null && <FadeInDown delay={transitionDuration}>
			<FormInput
		...
		<div  className={css`
			...
			right: ${rem(30)};
		`}>
			{queue === null && <FadeInUp delay={transitionDuration}>
				<Button
					type="button"
					onClick={async () => {
